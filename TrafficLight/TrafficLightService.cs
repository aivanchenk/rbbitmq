...
class TrafficLightService
{
	...
	/// Service logic.
	/// </summary>
	private TrafficLightLogic logic = new TrafficLightLogic();
	...
	private void OnMessageReceived(IModel channel, BasicDeliverEventArgs msgIn)
	{
		try
		{
			...
			switch( request.Action )
			{
				...
				case $"Call_{nameof(logic.GetLightState)}":
				{
					//make the call
					var result = logic.GetLightState();

					//create response
					response =
						new RPCMessage() {
							Action = $"Result_{nameof(logic.GetLightState)}",
							Data = JsonConvert.SerializeObject(new {Value = result})
						};
				...
				case $"Call_{nameof(logic.Queue)}":
				{
					//deserialize arguments
					var carDesc = JsonConvert.DeserializeObject<CarDesc>(request.Data);

					//make the call
					var result = logic.Queue(carDesc);

					//create response
					response =
						new RPCMessage() {
							Action = $"Result_{nameof(logic.Queue)}",
							Data = JsonConvert.SerializeObject(new {Value = result})
						};
				...
				case $"Call_{nameof(logic.IsFirstInLine)}":
				{
					//deserialize arguments
					var args = JsonConvert.DeserializeAnonymousType(request.Data, new {CarId = 1});

					//make the call
					var result = logic.IsFirstInLine(args.CarId);

					//create response
					response =
						new RPCMessage() {
							Action = $"Result_{nameof(logic.IsFirstInLine)}",
							Data = JsonConvert.SerializeObject(new {Value = result})
						};
				...
				case $"Call_{nameof(logic.Pass)}":
				{
					//deserialize arguments
					var carDesc = JsonConvert.DeserializeObject<CarDesc>(request.Data);

					//make the call
					var result = logic.Pass(carDesc);

					//create response
					response =
						new RPCMessage() {
							Action = $"Result_{nameof(logic.Pass)}",
